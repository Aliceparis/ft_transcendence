<script lang="ts">
    import { Login_Input, type LoginInput } from '$lib/shared/user.schema'; // 假设你定义了 Login_Input
    ...
    async function handleSubmit(event: SubmitEvent) {
        ...
        try {
            ...
            if (!response.ok) {
                // 根据后端实际返回的 AppError 错误码进行映射
                if (result.error?.code === "AUTH_INVALID_CREDENTIALS" || result.error?.code === "AUTH_USER_NOT_FOUND") {
                    // 安全起见，通常模糊提示或直接归于密码/邮箱错误
            ...
            // 登录成功（后端会通过 Set-Cookie 写入 auth_token）
